...
Do not write or update note files by default during a learn-buddy session. Answer
the user's questions in chat first, including follow-up questions, corrections,
and drill discussion.

Only create a Markdown note after the user explicitly says **"take notes"**.
When that happens, summarize the useful material from the session so far into a
concise one-pager. Do not include workflow/meta commands unless the user
explicitly asks for them to be recorded.

Save to: `${LEARN_BUDDY_INBOX:-~/Documents/work/0_inbox}/<concept>.md`

Use a descriptive lowercase hyphenated filename based on the concept, for example:

```text
${LEARN_BUDDY_INBOX:-~/Documents/work/0_inbox}/kv-cache.md
${LEARN_BUDDY_INBOX:-~/Documents/work/0_inbox}/rejection-sampling.md
${LEARN_BUDDY_INBOX:-~/Documents/work/0_inbox}/gradient-checkpointing.md
```

**Rules**:
- If the target filename already exists, append `-2`, `-3`, etc. Do not overwrite an existing note unless the user explicitly asks.
- Generate the note after the explicit "take notes" request, using the one-pager structure when it fits.
